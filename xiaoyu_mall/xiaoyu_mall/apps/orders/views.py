	...
	def get(self, request):
		...
		try:
			...
			if len(addresses) == 0:
				...
				# 构造上下文
				context = {
					'addresses': address_list   # 使用空列表代替空的 addresses
				}
		...
		# 取出所有的 sku
		for sku in skus:
			# 遍历 skus 给每一个 sku 补充 count（数量） 和 amount（小计）
			sku.count = new_cart_dict[sku.id]
			sku.amount = sku.price * sku.count  # Decimal 类型的
			# 累加数量和金额
...
class OrdersCommitView(LoginRequiredMixin, View):
	...
	def post(self, request):
		...
		try:
			...
			print(' 参数 address_id 错误 ')
			return HttpResponseForbidden(' 参数 address_id 错误 ')
		# 判断 pay_method 是否合法
		if pay_method not in [OrderInfo.PAY_METHODS_ENUM['CASH'], OrderInfo.PAY_METHODS_ENUM['ALIPAY']]:
			print(' 参数 pay_method 错误 ')
			return HttpResponseForbidden(' 参数 pay_method 错误 ')
		# 获取登录用户
		user = request.user
		# 获取订单编号： 时间 + user_id
		...
		with transaction.atomic():
			...
			try:
				...
				order = OrderInfo.objects.create(
					...
					status = OrderInfo.ORDER_STATUS_ENUM['UNPAID'] if pay_method == OrderInfo.PAY_METHODS_ENUM['ALIPAY'] else OrderInfo.PAY_METHODS_ENUM['UNSEND']
				)
